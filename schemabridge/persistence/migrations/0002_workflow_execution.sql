...
ALTER TABLE migration_workflows
    ADD CONSTRAINT migration_workflows_status_check CHECK (status IN (
        ...
        'VALIDATION_READY','VALIDATING','VALIDATED','FAILED','CANCELLED'
    ));
...
ALTER TABLE migration_workflow_artifacts
    ADD CONSTRAINT migration_workflow_artifacts_artifact_type_check CHECK (
        artifact_type IN (
            ...
            'APPROVED_MAPPING_PLAN','TRANSFORMATION_PREVIEW','EXECUTION_EVIDENCE',
...
ALTER TABLE migration_idempotency
    ADD CONSTRAINT migration_idempotency_command_type_check CHECK (
        command_type IN (
            'CREATE_WORKFLOW','TRANSITION_STATUS','APPEND_ARTIFACT','EXECUTE_WORKFLOW'
        )
    );

CREATE TABLE IF NOT EXISTS migration_execution_attempts (
    ...
    transformation_preview_artifact_version BIGINT NOT NULL CHECK (transformation_preview_artifact_version > 0),
    target_profile_id VARCHAR(256) NOT NULL,
    execution_fingerprint CHAR(64) NOT NULL CHECK (execution_fingerprint ~ '^[0-9a-f]{64}$'),
    status VARCHAR(32) NOT NULL CHECK (status IN ('CLAIMED','RUNNING','SUCCEEDED','FAILED_ROLLED_BACK','OUTCOME_UNCERTAIN')),
...
COMMENT ON TABLE migration_execution_attempts IS
    'Durable target execution claims and sanitized terminal outcomes.';
CREATE INDEX IF NOT EXISTS ix_migration_execution_attempts_workflow
    ON migration_execution_attempts(workflow_id, claimed_at);
CREATE UNIQUE INDEX IF NOT EXISTS ux_migration_execution_active_fingerprint
    ON migration_execution_attempts(workflow_id, execution_fingerprint)
