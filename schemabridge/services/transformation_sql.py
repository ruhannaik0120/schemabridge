"""Compile approved mapping expressions into deterministic Snowflake SQL.

The compiler is a pure boundary between a human-approved mapping plan and an
executable transformation preview.  It quotes all identifiers, binds literal
values as parameters, restricts expressions to approved source columns, and
rejects cyclic or excessively deep expression trees.
"""
...
class TransformationCompilationError(ValueError):
    """Base error for a transformation plan that cannot be rendered safely."""


class UnsupportedTransformationError(TransformationCompilationError):
    """Raised when an otherwise valid plan requests an unsupported expression."""


class InvalidTransformationPlanError(TransformationCompilationError):
    """Raised when the approved plan violates compiler invariants."""
...
def _q(value: Any) -> str:
    """Return a safely double-quoted SQL identifier component."""
...
def _relation(database: Any, schema: Any, table: Any) -> str:
    """Render a fully qualified relation from three independently quoted names."""

    return ".".join((_q(database), _q(schema), _q(table)))
...
def _expr(
    ...
    """Render one approved expression while collecting bound parameters.

    ``active`` identifies cycles in the current recursion path and ``depth``
    caps adversarial nesting.  Literal values and concat separators are never
    interpolated into SQL; their order in ``params`` matches placeholder order.
    """

    if depth >= _MAX_DEPTH or id(expression) in active:
        raise UnsupportedTransformationError(
            "Unsupported transformation expression."
        )
    if any(column not in allowed for column in expression.source_columns):
        raise InvalidTransformationPlanError("Invalid transformation plan.")

    column = lambda name: f"{_q(alias)}.{_q(name)}"
...
class SnowflakeTransformationSqlCompiler:
    """Render approved column transformations for the Snowflake dialect."""
    ...
    def compile_select(
        ...
        *,
        ...
        """Compile a read-only projection from a Snowflake staging relation."""
    ...
    def compile_insert_select(
        ...
        *,
        ...
        """Compile an insert-select from Snowflake staging into the target."""
    ...
    def _compile(self, plan, db, schema, table, alias, insert):
        """Validate plan invariants and render the selected statement shape."""
        ...
        approved = [
            ...
            if item.status
            in {MappingApprovalStatus.APPROVED, MappingApprovalStatus.OVERRIDDEN}
        ]
        if not approved or len({item.target_column for item in approved}) != len(
            approved
        ):
            raise InvalidTransformationPlanError("Invalid transformation plan.")

        # Target ordinal order makes the generated statement stable and keeps
        # its INSERT column list aligned with the SELECT projections.
        approved.sort(
            key=lambda item: (
        ...
        source_relation = _relation(db, schema, table)
        ...
        if insert:
            ...
            sql = (
                ...
                + ",\n    ".join(
                    f"{item} AS {_q(target)}"
                    for item, target in zip(projections, targets)
                )
                + "\nFROM "
                + source_relation
                + " AS "
        ...
        return GeneratedTransformationSql(
            ...
            source_relation=(db, schema, table),
            target_relation=(
                plan.target_table.catalog_name,
                plan.target_table.schema_name,
                plan.target_table.table_name,
...
def compile_snowflake_select(
    plan: ApprovedTableMappingPlan, **kwargs
) -> GeneratedTransformationSql:
    """Compile a Snowflake projection with a fresh stateless compiler."""

    return SnowflakeTransformationSqlCompiler().compile_select(plan, **kwargs)


def compile_snowflake_insert_select(
    plan: ApprovedTableMappingPlan, **kwargs
) -> GeneratedTransformationSql:
    """Compile a Snowflake insert-select with a fresh stateless compiler."""

    return SnowflakeTransformationSqlCompiler().compile_insert_select(
        plan, **kwargs
    )
