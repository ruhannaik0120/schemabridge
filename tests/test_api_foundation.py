...
from schemabridge.api import __version__
from schemabridge.api.app import create_app
from schemabridge.api.config import ApiSettings
from schemabridge.api.dependencies import get_schema_mapping_service
from schemabridge.api.errors import ApiError
from schemabridge.api.middleware import PlatformMiddleware, safe_request_id
from schemabridge.api.schemas.common import ApiSchema, BoundedText
    ...
    completed = subprocess.run(
        [
            ...
            (
                ...
                "factory=import_from_string('schemabridge.api.app:create_app'); "
                "assert factory().title == 'SchemaBridge API'; "
                "assert 'schemabridge.config' not in __import__('sys').modules; "
                "assert 'schemabridge.services.database_service' not in __import__('sys').modules; "
                "dependency=__import__('schemabridge.api.dependencies', fromlist=['get_schema_mapping_service']); "
...
def test_import_and_openapi_do_not_load_database_runtime() -> None:
    ...
    forbidden = {
        "schemabridge.config",
        "schemabridge.services.database_service",
        "schemabridge.connectors.postgresql.connector",
        "schemabridge.connectors.snowflake.connector",
    }
...
def test_shutdown_closes_only_an_already_loaded_supported_cache(monkeypatch) -> None:
    calls: list[str] = []
    fake = ModuleType("schemabridge.services.database_service")
    fake.reset_database_services = lambda: calls.append("reset")  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "schemabridge.services.database_service", fake)
