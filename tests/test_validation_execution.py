...
def test_approval_gate_prevents_resolution(monkeypatch):
 monkeypatch.setattr('schemabridge.services.validation_execution.get_database_service',lambda _:(_ for _ in ()).throw(AssertionError()))
...
def test_malformed_multiple_rows_is_not_a_validation_mismatch(monkeypatch):
 ...
 monkeypatch.setattr('schemabridge.services.validation_execution.get_database_service',lambda key:Bad(key,{},[]))
