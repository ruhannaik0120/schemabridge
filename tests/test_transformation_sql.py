...
import pytest
from schemabridge.models.mapping import MappingApprovalStatus, MappingReviewDecision, TransformationExpression, TransformationExpressionType
...
from schemabridge.services.transformation_sql import compile_snowflake_select, compile_snowflake_insert_select, InvalidTransformationPlanError
from tests.test_mapping_approval import _column, _table

def _approved():
    source=_table('source',_column('first_name',ordinal=1),_column('last_name',ordinal=2),_column('age',canonical_type=__import__('schemabridge.models.metadata',fromlist=['CanonicalType']).CanonicalType.INTEGER,ordinal=3))
    target=_table('people',_column('full_name',ordinal=1),_column('age',canonical_type=__import__('schemabridge.models.metadata',fromlist=['CanonicalType']).CanonicalType.INTEGER,ordinal=2))
