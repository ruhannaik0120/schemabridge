...
def test_docker_package_is_non_root_health_checked_and_migration_gated() -> None:
    ...
    assert "clean_mcp.api.app:create_app" in dockerfile
    for service in ("control-plane:", "migrate:", "api:"):
        assert service in compose
    assert "service_healthy" in compose
    assert "service_completed_successfully" in compose
    assert "schemabridge-control-plane:" in compose
    assert "migrate_control_plane.py" in compose
    assert "/health/ready" in compose
    assert "clean_mcp/tests" in ignore and "**/.env" in ignore
...
def test_environment_examples_separate_control_source_target_and_have_no_demo_passwords() -> None:
    ...
    nested = (WORKSPACE / "clean_mcp" / ".env.example").read_text(encoding="utf-8")
    combined = root + nested
    for name in (
...
def test_migration_cli_discovers_all_versions_without_a_database() -> None:
    completed = subprocess.run(
        [sys.executable, "clean_mcp/scripts/migrate_control_plane.py", "--check"],
        cwd=WORKSPACE,
        capture_output=True,
        text=True,
        check=False,
...
def test_recruiter_and_demo_documentation_match_implemented_scope() -> None:
    ...
    interview = (WORKSPACE / "clean_mcp/docs/INTERVIEW_DEMO.md").read_text(encoding="utf-8")
    workflow = (WORKSPACE / "clean_mcp/docs/LOCAL_WORKFLOW_DEMO.md").read_text(encoding="utf-8")
    for term in (
        "SchemaBridge",
        "```mermaid",
        "Idempotency-Key",
